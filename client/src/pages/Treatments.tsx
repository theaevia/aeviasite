...
export default function Treatments() {
  ...
  useEffect(() => {
    const scrollToHash = () => {
      const hash = window.location.hash.slice(1);          // "mind"
      if (!hash) return;

      // small timeout lets the layout finish before we scroll
      setTimeout(() => {
        const element = document.getElementById(hash);
        if (element) {
          const navHeight = 80; // Approximate height of the fixed nav bar
    ...
    scrollToHash();                           // ── on first render
    window.addEventListener("hashchange", scrollToHash); // ── if the user clicks the link again

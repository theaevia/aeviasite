...
function buildTreatmentCrawlableContent(slug: string, heading: string, description: string): string | null {
  ...
  const courseOffer = ({
    profhilo: { title: 'Profhilo® Glow Protocol', price: pricing.protocols.glow.display, plan: pricing.protocols.glow.paymentDisplay },
    'full-face-regeneration': { title: 'Plinest Full Face Regeneration Course', price: pricing.regenerative.plinestFace.courseDisplay, plan: pricing.regenerative.plinestFace.coursePaymentDisplay },
    'eye-rejuvenation': { title: 'Plinest Eye Revival Course', price: pricing.protocols.eyeRevival.display, plan: pricing.regenerative.plinestEye.coursePaymentDisplay },
    sunekos: { title: 'Sunekos Skin Renewal Course', price: pricing.regenerative.sunekos.courseDisplay, plan: pricing.regenerative.sunekos.coursePaymentDisplay },
  } as Record<string, { title: string; price: string; plan: string }>)[slug];
  const priceContent = courseOffer
    ? `<h3>Clinician-recommended plan: ${escapeHtml(courseOffer.title)}</h3><p>${escapeHtml(courseOffer.plan)}</p><p>Single session: ${escapeHtml(price)}. For maintenance, staged treatment or where advised after consultation.</p><p>If your clinician recommends a course after a single session, that session price can be put towards the course when you book it within 30 days.</p>`
...
const PRIORITY_PAGE_CONTENT: Record<string, Array<{ heading: string; paragraphs: string[] }>> = {
  '/treatments': [
    ...
    { heading: 'The Glow Protocol', paragraphs: [pricing.protocols.glow.paymentDisplay] },
